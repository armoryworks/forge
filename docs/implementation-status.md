...
| Carrier APIs (direct) | proposal.md §4.21 | Done | All four direct carriers implemented: UpsShippingService (OAuth 2.0 + Rating/Shipping/Tracking v2403), FedExShippingService (OAuth + Rates/Ship/Track), UspsShippingService (Addresses API v3 OAuth), DhlShippingService (DHL Express MyDHL API). MultiCarrierShippingService aggregator fans out across all configured carriers in parallel for rate shopping; routes label creation by carrierId prefix; auto-detects carrier from tracking-number format. All four wired in IntegrationDescriptorCatalog with setup steps + signup URLs so admin UI surfaces them on equal footing with SMTP/MinIO/etc. Stamps.com is in the descriptor + options + DI (admin can capture credentials hot-reload) but the actual SwsimV111 SOAP service is the only remaining gap. Address validation split to IAddressValidationService (USPS Web Tools). Handlers: GetShippingRates, CreateShippingLabel, GetShipmentTracking. Angular: ShippingRatesDialog + TrackingTimeline components. IOptions hot-reload on admin save covers UPS/FedEx/USPS/DHL/Stamps so credential changes take effect without a process restart. |
...
| 23 | Customer Portal | Done (v1) | Backend: CustomerPortalAccess entity + per-contact magic-link auth, CustomerPortalController with 7 endpoints (request-link / exchange / dashboard / sales-orders / quotes / invoices / shipments / quote respond), capability CAP-EXT-CUSTOMER-PORTAL (default OFF), AddCustomerPortalAccess migration, IPortalAuthService + PortalAuthService for SHA-256 hashed one-time tokens, ⚡ Standalone Financial enforcement on invoices endpoint (returns empty when external accounting provider connected). Frontend: /portal/* parallel UI living outside the employee app shell, login + auth callback pages, portal layout (no sidebar, distinct chrome), dashboard with KPI cards, four list pages (orders / quotes / invoices / shipments), quote accept/decline inline buttons. Portal session JWT carries portal_session=true + customer_id + contact_id claims; portal-token storage and portalAuthInterceptor live under separate localStorage namespace from employee qbe-token so the two never bleed. |

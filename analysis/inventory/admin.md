...
| ID | component | type | route | file:line | renders-for | states | purpose |
|----|-----------|------|-------|-----------|-------------|--------|---------|
| ADM-TERM-01 | terminology tab panel | tab | `/admin/terminology` | `admin.component.html:182` | A only | empty(live-7 confirmed): "0 ENTRIES · No terminology entries configured yet" + refresh icon; SAVE CHANGES button visible · populated(TODO — no entries in non-seeded env; D4) | Inline-editable table of terminology overrides (key → custom label) |

...
| PLT-Q-004 | Reports | All 28 report types (at minimum: jobs-by-stage, overdue-jobs, ar-aging) — sweep as admin@ | HIGH | ✅ PARTIALLY CLOSED — all 30 nav items confirmed live. 3 report types clicked (Jobs by Stage, Overdue Jobs, Time by User — all empty). ProductionWorker → redirected to /dashboard (confirmed). Engineer → redirected to /dashboard (confirmed in cycle 2 re-verify; cycle 1 "no redirect" was sweep false positive — see PLT-Q-033). Empty states only; populated states → PLT-Q-031. |
...
| PLT-Q-033 | Reports | Engineer role /reports access — cycle 1 reported no redirect (discrepancy vs `roleGuard('Admin','Manager','PM')` @ app.routes.ts:139) | HIGH | ✅ CLOSED — NOT a guard bypass. JWT for engineer@forge.local carries ONLY `"Engineer"` (`http://schemas.microsoft.com/.../role: "Engineer"`; `/auth/me` confirms `roles: ["Engineer"]`). Re-verified live: `/reports` nav as engineer redirects to `/dashboard` (correct). Cycle 1 false positive: sweep access check tested `!url.includes('/forbidden'/'login'/'unauthorized')` but guard redirects to `/dashboard` — that URL passed the check, creating a spurious "access=true" reading. Source verdict (d66aebf) confirmed: guard is correct and exclusive. No platform.md line warranted. |
...
| PLT-Q-033 | 2026-05-22 | Engineer /reports access — NOT a bypass. JWT carries only "Engineer"; guard correctly redirects to /dashboard. Cycle 1 was false positive (access check didn't match /dashboard as redirect target). |
